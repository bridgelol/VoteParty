...
class VoteParty internal constructor(internal val plugin: VotePartyPlugin) : State {
    ...
    private fun loadHook() {
        this.hook =
            if (Bukkit.getBukkitVersion().substringBefore('-').substringAfter('.').substringBefore('.').toInt() >= 13) {
                VersionHookNew()
            } else {
                VersionHookOld()
            }
    ...
    private fun logo(sender: ConsoleCommandSender) {
        val logo = LOGO.replace("{plugin_version}", plugin.description.version)
            .replace("{server_version}", plugin.server.version)
    ...
    @Deprecated(
        "This was only used in the PlaceholderAPI Placeholders. A new and improved method has been implemented",
        ReplaceWith("UserHandler#getVotesWithinRange")
    )
